# 空间转录组怎么选平台、怎么做分析？一份整合指南

> 整合解读 | Lim H J, et al. *BMC Genomics*, 2025 + Fred Hutch *Choosing Genomics Tools* Ch.14  
> 平台指南：[DOI: 10.1186/s12864-025-11235-y](https://doi.org/10.1186/s12864-025-11235-y)  
> 分析教程：[Fred Hutch Spatial transcriptomics](https://hutchdatascience.org/Choosing_Genomics_Tools/spatial-transcriptomics-1.html)  
> 本地平台笔记：[0.platform/README.md](./0.platform/README.md)
...
空间转录组（Spatial Transcriptomics, ST）正是为补上这块拼图。Fred Hutch 数据科学实验室的 *Choosing Genomics Tools* 梳理了**分析逻辑与工具地图**；*BMC Genomics* 2025 的七大主流平台选型指南（Lim et al.）则给出了**实验层面的横向对比**。本文将两者整合，形成「先选平台、再跑流程」的实用框架。

---

## 一、先理解核心权衡：空间分辨率 vs 分子分辨率
...
## 二、主流平台三分法（BMC Genomics 2025）

Lim 等将七大（含新兴）商业平台按**技术路线 + 分辨率**分为三组：
...
| 平台 | 更新要点 |
|------|----------|
| **StrataMap Spatial** | 2026 年 6 月正式商业化（RUO）；1 μm 特征密度；当前仅 FF，FFPE 预计 2027 |
| **Xenium Prime 5K** | ~5000 基因预制 panel + ≤100 自定义；Xenium Protein 同片 RNA+蛋白 |
| **CosMx WTX** | ~18,000–19,000 蛋白编码基因准全转录组成像 |
| **MERSCOPE** | 全定制 ≤1000 基因任意物种无设计费；支持培养细胞/类器官 |

---

## 三、快速选型：你的需求对应哪个平台？

结合 BMC 指南 Table 2 与 Fred Hutch 的分析视角：

| 你的需求 | 优先考虑 |
|----------|----------|
| 无偏全转录组 + 最快上手 | **10x Visium**（G1） |
| 已知 ROI / 病理圈选 / 多区域比较 | **GeoMx**（G1） |
| 全转录组 + 单细胞尺度 + FFPE 生态成熟 | **Visium HD**（G2） |
| 任意物种 + 超大组织 + 纳米级 | **Stereo-seq**（G2） |
| 全转录组 + 1 μm 分辨率 + 大捕获面积 | **StrataMap Spatial**（G2）★ |
| 靶向 panel + 亚细胞 + FFPE 降解样本 | **Xenium**（G3） |
| 靶向 panel + 任意物种开放定制 | **MERSCOPE**（G3） |
| 6K 高多重或 ~18K 准全转录组成像 | **CosMx WTX**（G3） |
...
7. 10x Genomics. Xenium In Situ Platform. https://www.10xgenomics.com/platforms/xenium

8. Bruker Spatial Biology. CosMx Spatial Molecular Imager. https://brukerspatialbiology.com/products/cosmx-spatial-molecular-imager/
